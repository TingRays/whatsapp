...
@section('container')
    {!!
        ...
        ->setTabs(function (\Abnermouke\Pros\Builders\Table\Tools\TableTabBuilder $tabBuilder) use ($bm_id) {
            $tabBuilder->create('all', '全部', route('whatsapp.console.merchant.lists',['bm_id' => $bm_id]));
            $tabBuilder->create('enabled', '正常启用', route('whatsapp.console.merchant.lists', ['bm_id' => $bm_id,'status' => \App\Model\Pros\WhatsApp\Merchants::STATUS_ENABLED]));
            $tabBuilder->create('disabled', '已禁用', route('whatsapp.console.merchant.lists', ['bm_id' => $bm_id,'status' => \App\Model\Pros\WhatsApp\Merchants::STATUS_DISABLED]));
        })
        ->setButtons(function (\Abnermouke\Pros\Builders\Table\Tools\TableButtonBuilder $buttonBuilder) use ($bm_id) {
            $buttonBuilder->form(route('whatsapp.console.merchant.detail', ['bm_id' => $bm_id,'id' => 0]), '添加商户')->theme('info');
        })
        ->setActions(function (\Abnermouke\Pros\Builders\Table\Tools\TableActionBuilder $actionBuilder) use ($bm_id) {
            $actionBuilder->form(route('whatsapp.console.merchant.detail', ['bm_id' => $bm_id,'id' => '__ID__']), '编辑商户')->icon('fa fa-edit');
        })
